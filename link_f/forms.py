...
#    status = forms.CharField(max_length=100, label="Status", widget=forms.Textarea(attrs={'rows':1, 'style': 'width: 300px;'}))
    ...
    hub_ip = forms.GenericIPAddressField(label='HUB IP', protocol='both', required=True, widget=forms.TextInput())

class ChangePassword(forms.Form):
    new_password = forms.CharField(label='New Password', widget=forms.PasswordInput(attrs={'style': 'width: 300px;'}))
    confirm_password = forms.CharField(label='Confirm Password', widget=forms.PasswordInput(attrs={'style': 'width: 300px;'}))
    def clean(self):
...
class WANSettingsForm(forms.Form):
