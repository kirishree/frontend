...
vrf_name = "vrf1"
table_id = "10"
tunnel_network  = "10.200.202.0/24"
...
def get_vrf_gateway():
    try:
        # Use `ip route` to fetch routes for the specified VRF table
        command = f"ip route show table {table_id} | grep default"
        result = subprocess.check_output(command, shell=True, text=True).strip()

        # Extract gateway from the output (assuming standard output format)
        gateway = result.split()[2]  # 'default via <gateway>'        
        return gateway
    except subprocess.CalledProcessError as e:
        print(f"Error: Could not retrieve gateway for VRF table {vrf_table}.")
        print(e.output)
        return None
    except IndexError:
        print("No gateway found in the VRF table output.")
        return None
...
def check_tunnel_connection():
    try:    
        ...
        dft_gw = get_vrf_gateway()
        if dft_gw != "10.200.202.2" :
            os.system(f"sudo ip route replace table {table_id} default via 10.200.202.2")    
        return True      
...
def get_tunnel_ip(Local_public_ip):
    ...
    for info in coll_tunnel_ip.find({},{'_id':0}):
        if info["uuid"] == sys_uuid:
            ...
            subnet = info["subnet"]
            query = {"uuid": sys_uuid}
            update_data = {"$set": {"public_ip":Local_public_ip                  
                                    }
                            }
...
def create_tunnel(Local_tunnel_ip, default_gateway):
    ...
    status = run_command(command)
    command = f"sudo ip link add dev {vrf_name} type vrf table {table_id}"
    status = run_command(command)
    command = f"sudo ip link set up dev {vrf_name}"
    status = run_command(command)
    command = f"sudo ip link set dev Reach_link1 master {vrf_name}"
    status = run_command(command)
    command = f"sudo ip route add table 10 default via {Remote_tunnel_ip}"
    status = run_command(command)
    command = f"sudo ip route add {tunnel_network} dev {vrf_name}"
    status = run_command(command)
    command = f"systemctl restart linkbe"
    status = run_command(command)
...
def create_setup():    
    ...
    if Local_public_ip != False:    
        ...
        with open(file_path, "w") as f:
            ...
            f.close()    
        tunnel_created = create_tunnel(Local_tunnel_ip, default_gateway)
        if tunnel_created:            
    ...
    while(1):
        ...
        if status == False:
            ...
            if New_public_ip != False:
                ...
                coll_tunnel_ip.delete_many({"uuid": system_uuid})
